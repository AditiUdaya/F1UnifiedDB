...
const Races = () => {
  ...
  // ✅ new state for analytics (SQL-based)
  ...
  useEffect(() => {
    const fetchRacesAndAnalytics = async () => {
      try {
        ...
        const analyticsRes = await fetch("http://localhost:5000/races/analytics");
        if (analyticsRes.ok) {
          const analyticsData = await analyticsRes.json();
          setAnalytics({
            totalRaces: analyticsData.TotalRaces || 0,
            earliestRace: analyticsData.EarliestRace
              ? new Date(analyticsData.EarliestRace).toLocaleDateString()
              : "N/A",
            latestRace: analyticsData.LatestRace
              ? new Date(analyticsData.LatestRace).toLocaleDateString()
              : "N/A",
          });
        }
      } catch (err) {
        console.error("Error fetching races or analytics:", err);
    ...
    <div className="space-y-6 p-6">
      ...
      >
        ...
        <Button className="gap-2 bg-gradient-racing font-medium racing-glow-sm hover:opacity-90">
          {/* kept button empty intentionally */}
        </Button>
      </motion.div>
      ...
      {/* ✅ Analytics Section (updated to use backend data) */}
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
      ...
      >
        <Card className="border-accent/20 bg-card-elevated p-6">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            Race statistics calculated using SQL aggregate functions (COUNT, MIN, MAX)
          </p>
          <p className="text-sm text-muted-foreground">
            Race details and analytics use a LEFT JOIN in SQL to ensure all races appear.
          </p>
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-3">
            <div className="rounded-lg border border-border bg-muted/50 p-4">
              ...
              <p className="mt-1 font-orbitron text-2xl font-bold text-accent">
                {analytics.totalRaces}
              </p>
            </div>
            <div className="rounded-lg border border-border bg-muted/50 p-4">
              <p className="text-xs text-muted-foreground">Earliest Race</p>
              <p className="mt-1 font-orbitron text-xl font-bold text-accent">
                {analytics.earliestRace}
              </p>
            </div>
            <div className="rounded-lg border border-border bg-muted/50 p-4">
              <p className="text-xs text-muted-foreground">Latest Race</p>
              <p className="mt-1 font-orbitron text-xl font-bold text-accent">
                {analytics.latestRace}
              </p>
            </div>
          </div>
        </Card>
